...
add_action( 'after_setup_theme', 'snappy_theme_setup', 10 );
function snappy_theme_setup() {
add_action('wp_enqueue_scripts', 'navid_rtl_css');	
}


function snappychild_translation_setup() {
	load_child_theme_textdomain( 'snappy', get_stylesheet_directory() . '/languages');
}
add_action( 'after_setup_theme', 'snappychild_translation_setup' );
