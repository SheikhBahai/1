...
if (!class_exists('Snappy_Walker_Page')) {
    /**
     * CUSTOM PAGE WALKER
     * A custom walker for pages.
     */
    class Snappy_Walker_Page extends Walker_Page
    {
        ...
        public function start_el(&$output, $page, $depth = 0, $args = array(), $current_page = 0)
        {
            ...
            if ('' === $page->post_title) {
                ...
                $page->post_title = sprintf(__('#%d (no title)', 'snappy'), $page->ID);
            }
            ...
            if (isset($args['show_sub_menu_icons']) && true === $args['show_sub_menu_icons']) {
                if (isset($args['pages_with_children'][$page->ID])) {
                    $args['icon_rennder'] = '<span class="icon">'.snappy_the_theme_svg('chevron-down',true).'</span>';
                }
            }
            ...
            if (isset($args['show_toggles']) && true === $args['show_toggles']) {
                if (isset($args['pages_with_children'][$page->ID])) {
                    ...
                    $args['list_item_after'] = '<button type="button" class="theme-aria-button submenu-toggle" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="250"><span class="btn__content" tabindex="-1"><span class="screen-reader-text">' . __( 'Show sub menu', 'snappy' ) . '</span>' . snappy_get_theme_svg( 'chevron-down' ) . '</span></button>';
                }
            }
