...
class Snappy_Walkernav extends Walker_Nav_Menu
{
    ...
    {   
        ...
        $item_output .= $link_before.apply_filters('the_title', $item->title, $item->ID).$link_after;
            ...
            if( isset( $args->has_children ) && !$args->has_children && ( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) && $depth == 0 ) ) {
                ...
                $item_output .= '<span class="icon">'.snappy_the_theme_svg('chevron-down',true).'</span>';

            }
            // add amche for menu item descriptions
        ...
        $item_output .= ( ( $depth == 0 || 1 ) && $has_children ) ? '<span class="icon">'.snappy_the_theme_svg('chevron-down',true).'</span></a>' : '</a>';
