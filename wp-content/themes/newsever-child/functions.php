...
add_action( 'after_setup_theme', 'newsever_theme_setup', 10 );
function newsever_theme_setup() {
add_action('wp_enqueue_scripts', 'navid_rtl_css');	
}


function newseverchild_translation_setup() {
	load_child_theme_textdomain( 'newsever', get_stylesheet_directory() . '/languages');
}
add_action( 'after_setup_theme', 'newseverchild_translation_setup' );
