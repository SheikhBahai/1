<?php
if ( !class_exists('Snappy_Getting_started') ):

    class Snappy_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_snappy_install_plugins', array( $this, 'snappy_install_plugins' ) );
        ...
        public static function snappy_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','snappy') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','snappy') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','snappy') );
        ...
        public static function snappy_recommended_plugins(){
        ...
        public function snappy_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'snappy' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'snappy_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'snappy') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->snappy_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->snappy_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'snappy');
                            die();
                        }
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=snappy-about' );
            }
